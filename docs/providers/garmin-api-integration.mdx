...
<Steps>
  ...
  <Step title="Create an Evaluation app (this gives you Client ID + Client Secret)">
    ...
    - Select the APIs you intend to use: **Health** and **Activity**. Do **not** select **Women’s Health** — it is a separate API product covering menstrual cycle and pregnancy tracking, which is out of scope for this deployment (see the next step)
  ...
  <Step title="Configure API Tools (endpoints + data types/scopes)">
    ...
    **Which endpoints to enable:**
    ...
    Enable exactly these endpoints — and **not `mct`**, which would subscribe this deployment to menstrual cycle and pregnancy data it is not set up to hold lawfully:
    ...
    These are internal names, so if a row on Garmin’s page is labelled differently — `userPermissionsChange` and `deregistrations` especially — match it by meaning and enable the equivalent row, with the sole exception of `mct`, which stays off whatever Garmin calls it.

    <Warning>
    **Why `mct` (Menstrual Cycle Tracking) is left off.**

    Menstrual cycle and pregnancy tracking are data concerning health and sex life under **Art. 9 GDPR** — special categories of personal data — and they are out of scope for this deployment.

    Enabling the endpoint is not a passive choice. Garmin pushes the data, Open Wearables writes it to the `menstrual_cycle_details` table, and it dispatches a `menstrual_cycle.created` outgoing webhook whose payload carries a `pregnancy_snapshot`. That places special-category data in your database and in every downstream subscriber, with no Art. 9 legal basis, no consent flow and no DPIA covering it.

    If your deployment genuinely needs this data, treat it as a separate decision with its own legal review — not as part of “enable everything”.
    </Warning>
