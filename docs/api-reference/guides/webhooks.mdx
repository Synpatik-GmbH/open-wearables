...
<Note>
Open Wearables never gives Svix a readable user id. Each message's user channel is stored in Svix as an opaque, per-user token, and the endpoints and messages APIs translate it back, so you keep seeing `user_id` and `user.<uuid>` channels in responses. Endpoints scoped to a user before this change are moved to the opaque form automatically when the API starts, so their filter keeps matching new messages. Svix applies a changed endpoint only after refreshing its cached configuration, so an event sent in the moments right after startup may still be routed by the old filter.
</Note>
