...
<Note>
Open Wearables never gives Svix a readable user id. Each message's user channel is stored in Svix as an opaque, per-user token, and the endpoints and messages APIs translate it back, so you keep seeing `user_id` and `user.<uuid>` channels in responses. **Upgrading a self-hosted installation:** an endpoint scoped to a user before this change still filters on the readable channel and receives nothing until migrated. Run [the Svix user-channel migration](/data-migrations/pseudonymise-svix-user-channels) once — `docker compose exec app uv run python scripts/data_migrations/pseudonymise_svix_user_channels.py --dry-run` to see how many exist, then without `--dry-run` to migrate them. It is idempotent, and exits non-zero if Svix could not be reached, in which case rerun it. Svix applies a changed endpoint only after refreshing its cached configuration, so an event sent in the moments right after the migration may still be routed by the old filter.
</Note>
...
- **Rotating `SVIX_PSEUDONYM_SECRET`.** Every digest changes, so events sent before the rotation stop deduplicating against new ones. Rotation also changes every user channel: an endpoint scoped to a user receives nothing, and reports `user_id` as `null` as if it were unscoped, until its `user_id` is saved again. Set the secret explicitly before the first event in production, and rotate it only deliberately.
</Warning>
